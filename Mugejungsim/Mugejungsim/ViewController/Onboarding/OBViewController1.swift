...
class OBViewController1: UIViewController {

    // dot1 이미지 뷰
    private let dotImageView: UIImageView = {
        let imageView = UIImageView()
        imageView.image = UIImage(named: "dots1") 
    ...
    @objc private func nextButtonTapped() {
        let viewController2 = OBViewController2()
        
        // 새 화면을 기존 뷰와 부드럽게 교체
        UIView.transition(with: self.view.window!, duration: 0.3, options: .transitionCrossDissolve, animations: {
            UIApplication.shared.windows.first?.rootViewController = viewController2
        }, completion: nil)
    }
}
